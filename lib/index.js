...
program
  ...
  .action(async () => {
    const res = await getOrigin();
    console.log('%c [ res ]-61', 'font-size:13px; background:#b62b13; color:#fa6f57;', res)
  ...
  .action(() => {
    inquirer
      ...
      .then((result) => {
        ...
        console.log('%c [ reg ]-87', 'font-size:13px; background:#ad7cf5; color:#f1c0ff;', reg)
...
program
  .command("delete")
  .alias("d")
  .description("delete custom source")
  .action(
    () => {
      const keys = Object.keys(registries);
...
program
  ...
  .description("create project into folder, choose framework")
  .action((project_name) => {
    const keys = Object.keys(framework);
    inquirer
      .prompt([
        {
          type: "list",
          name: "sel",
          message: "please switch framework",
          choices: keys,
        },
      ])
      .then((result) => {
        download(framework[result.sel], project_name, { clone: true }, (err) => {
          if (err) {
            console.log(chalk.red("create fail:", err));
          } else {
